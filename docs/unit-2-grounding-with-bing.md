...
Right now, your Onboarding Assistant can only respond using the knowledge baked into its underlying language model. That means it can't look up a prospective client's company, check current industry regulations, or find the latest compliance requirements. By adding Bing as a tool, your agent will pull in current, relevant information from the web — and cite its sources.
...
By the end of this unit, your agent will be able to answer questions about the real world with up-to-date, grounded responses — turning it from a generic chatbot into a research-capable assistant.
...
- ✅ Your **Onboarding-Assistant** agent is working in the Foundry playground
...
- Provide outdated compliance requirements confidently
- "Hallucinate" plausible-sounding but incorrect industry information
...
> **💡 Tip:** Think of Grounding with Bing as giving your agent a web browser. Instead of guessing, it can look things up — just like you would when researching a new client or checking the latest regulations.
...
Now let's connect the Bing Grounding resource to your Onboarding Assistant.
...
3. Select the **Onboarding-Assistant** agent you created in Unit 1 to open its configuration.
...
Time to see the difference Bing Grounding makes! Let's test with questions that require current information — the kind of research you'd do when onboarding a new client.

1. In the Foundry portal, open the **playground** for your Onboarding-Assistant.
2. Start by researching a prospective client:

   ```
   I'm onboarding a new client — Contoso Construction Ltd. What can you tell me about this company?
   ```
...
4. Now try looking up current regulations — a common need when onboarding clients in regulated industries:

   ```
   What are the current OSHA safety requirements for commercial construction projects in 2026?
   ```

   ```
   What are the latest changes to AML (Anti-Money Laundering) regulations for professional services firms?
   ```
...
5. Compare this to Unit 1. Remember when you asked about specific policies and the agent gave generic answers? Try this:
   ...
   ```
   What are the key compliance considerations when onboarding a construction client in Portland, Oregon?
   ```
...
6. Also try a question that *doesn't* need web search to see that the agent still behaves normally:
   ...
   The agent should still respond as your Onboarding Assistant — Bing Grounding doesn't replace the agent's core behavior, it **extends** it.

> **💡 Tip:** Pay attention to the citations in the agent's responses. Grounded answers will often include links or references to the web sources used. This transparency is critical in professional services — you can verify the information before relying on it for client work.
...
Congratulations! 🎉 You've added real-time web knowledge to your Onboarding Assistant. Here's what you accomplished:

| ✅ Done | ❌ Not Yet |
|---|---|
| Created a declarative agent in Foundry (Unit 1) | No firm-specific knowledge (policies, procedures) |
| Added Grounding with Bing for real-time web knowledge | No tools connected (e.g., project tracker) |
...
- **Tools in Foundry** — Pluggable capabilities that can be added to a declarative agent through the Foundry UI. Grounding with Bing is one example; others include file uploads, Azure AI Search indexes, MCP connections, and more.
